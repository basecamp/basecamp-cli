...
process_project() {
  ...
  project_data=$(api_call "$BCQ_API_BASE/projects/$project_id.json")
  ...
  # 2. Get all todolists
  todolists=$(api_call "$BCQ_API_BASE/buckets/$project_id/todosets/$todoset_id/todolists.json")
  ...
  for list_id in $todolist_ids; do
    ...
    page=1
    while true; do
      todos=$(api_call "$BCQ_API_BASE/buckets/$project_id/todolists/$list_id/todos.json?page=$page")
