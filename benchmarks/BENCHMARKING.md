...
Final claims require N>=5 valid runs per model/strategy cell. If N differs,
...
| Gate | When | What |
|------|------|------|
| **Smoke** | Any CLI/skill change | Task 12, bcq strategy, Haiku, 1 run. Must pass. |
...
```bash
source env.sh
./harness/run.sh --task 12 --strategy bcq --model claude-haiku
