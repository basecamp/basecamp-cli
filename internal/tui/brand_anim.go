...
func AnimateWordmarkWith(w io.Writer, theme Theme, strategy string) {
	...
	// Batch cells into animation frames
	...
	// State: frame at which each cell was revealed (-1 = not yet visible)
...
func traceRadial(grid [][]rune, numLines int) []paintCell {
	...
	sort.Slice(cells, func(i, j int) bool {
		di, dj := cells[i].dist, cells[j].dist
		// Bin distances to create rings (~2 cell wide)
		bi, bj := int(di/2.0), int(dj/2.0)
		if bi != bj {
			return bi < bj
		}
