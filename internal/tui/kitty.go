...
// kittyUploadAndPlace returns escape sequences to upload image data and create a
// virtual Unicode placement. The result goes to the terminal through tea.Raw.
...
// by the terminal and the message shows an empty gap where its image was. Anything
// that is not already a PNG is re-encoded, at the fastest compression setting since
// this runs while the screen is being drawn.
