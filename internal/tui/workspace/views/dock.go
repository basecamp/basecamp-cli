...
func NewDock(session *workspace.Session, store *data.Store, projectID int64) *Dock {
	...
	if p := store.Project(projectID); p != nil && len(p.Dock) > 0 {
		v.project = p
		v.syncTools()
	} else {
		v.loading = true
...
func (v *Dock) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case workspace.DockLoadedMsg:
		...
		v.store.UpsertProject(msg.Project)
		v.syncTools()
		return v, nil
...
func (v *Dock) syncTools() {
	...
	for _, tool := range v.project.Dock {
		if !tool.Enabled {
			continue
		}
		title := tool.Title
