...
// styleInlineMarkdown restyles a text field's rendered view so the Markdown being
...
// styling is added — every character and every sequence the field drew stays, and
// its own styling (the cursor, the cursor line) is restored where a span ends. A
// span styles only once its delimiters pair up on one visual line, so half-typed
// markup stays plain.
//
// Ported from hey-cli's internal/tui/inline_markdown.go.
...
// ansiSequenceAt reads one escape sequence starting at the ESC in rs[0]: a CSI
// sequence up to its final byte, an OSC up to its terminator, or the two-rune form
...
// delimiterRuns collects the maximal runs of a delimiter outside code spans. A run
// only opens against text (no space after it) and only closes against text (no
// space before it); wordEdge also demands a word boundary outside the run, which
// is what keeps snake_case plain.
...
// pairStyle matches openers to closers left to right among the runs that can carry
// the style: a run of the style's own width, or of three, which serves bold and
// italic at once (***both***). Unpaired delimiters stay plain.
...
// rebuildLine writes the line back with the spans styled. The field's own
// sequences pass through where they were; ours layer on top of them, re-asserted
// after any of theirs, and a span ends by resetting and replaying the SGR state the
// line had built up, so their styling is exactly restored.
...
// sgrFor styles a span the way internal/markdown styles a rendered message: slots
// 11 and 12 are the code and heading colors terminalStyle names there.
