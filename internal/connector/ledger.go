...
`,
	...
	// Migration 8. The git worktrees a task runs in: the row written before
	// `git worktree add`, the states it moves through, and the ones kept when
	// a task ends holding work. See ledger_worktrees.go.
	//
	// This was also 8 on the base this branch was written against, but behind
	// a different 7 — card 20's outbox, which has not landed. It follows the
	// dispatcher's tables at 7 now; the number stayed and the reason for it
	// moved. Whatever lands next takes 9: a shipped migration is never
	// renumbered under a ledger that has already applied it.
	migrationWorktrees,
}
