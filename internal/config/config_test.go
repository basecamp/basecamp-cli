...
// root.go's validateLLMEndpoint fail closed for non-config commands while config
// subcommands can still repair via `config unset llm_endpoint`.
...
// TestLoadFromEnv_LLMEndpointSchemeRejected verifies a non-http(s) or hostless
// BASECAMP_LLM_ENDPOINT fails closed: LoadFromEnv returns an error rather than
// warning and dropping the value, so a typo can't silently fall back to the
// default provider.
func TestLoadFromEnv_LLMEndpointSchemeRejected(t *testing.T) {
	for _, bad := range []string{"file:///etc/passwd", "https:example.com", "ssh://host/x"} {
		t.Run(bad, func(t *testing.T) {
			...
			require.Error(t, err)
			assert.Contains(t, err.Error(), "must be an http:// or https:// URL with a host")
			assert.Contains(t, err.Error(), "BASECAMP_LLM_ENDPOINT")
...
// TestIsHTTPURL covers the scheme+host validation reused by config-accept and
// the root.go llm_endpoint enforcement.
func TestIsHTTPURL(t *testing.T) {
	for _, ok := range []string{"http://localhost:11434", "https://api.openai.com/v1", "http://127.0.0.1:8080/x"} {
		assert.True(t, IsHTTPURL(ok), "%q should be accepted", ok)
	}
	for _, bad := range []string{"", "file:///etc/passwd", "ssh://h/x", "https:example.com", "https://", "not a url", "/relative"} {
