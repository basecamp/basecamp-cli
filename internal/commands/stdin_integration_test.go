...
func TestDeterministicFailuresRejectedBeforeReadingStdin(t *testing.T) {
	for _, tc := range []struct {
		...
		{"chat post bad content-type", NewChatCmd,
		...
		{"cards update bad attachment", NewCardsCmd,
			[]string{"update", "1", "--body", "-", "--attach", "/nope/missing.png"}, "missing.png"},
		{"chat update untrusted host", NewChatCmd,
			[]string{"update", "https://evil.example/1/buckets/2/chats/3/lines/4", "-"}, "untrusted host"},
		{"chat update wrong recording type", NewChatCmd,
			[]string{"update", "https://3.basecamp.com/1/buckets/2/todos/4", "-"}, "expected a chat-line"},
		{"chat update bad line id", NewChatCmd,
			[]string{"update", "nope", "-"}, "Invalid chat line ID"},
		{"files replace wrong recording type", NewFilesCmd,
			[]string{"replace", "https://3.basecamp.com/1/buckets/2/todos/4", "stdin.go", "--description", "-"}, "not an upload"},
		{"schedule create bad schedule id", NewScheduleCmd,
			[]string{"create", "Title", "--starts-at", "2026-01-01T10:00:00Z", "--ends-at", "2026-01-01T11:00:00Z", "--schedule", "nope", "--description", "-"}, "Invalid schedule ID"},
		{"messages create bad board id", NewMessagesCmd,
			[]string{"create", "Title", "-", "--message-board", "nope"}, "Invalid message board ID"},
		{"docs create bad folder id", NewDocsCmd,
			[]string{"documents", "create", "Title", "-", "--folder", "nope"}, "Invalid folder ID"},
		{"todolists create bad todoset id", NewTodolistsCmd,
			[]string{"create", "Title", "--description", "-", "--todoset", "nope"}, "Invalid todoset ID"},
		{"cards column create bad table id", NewCardsCmd,
			[]string{"column", "create", "Title", "--description", "-", "--card-table", "nope"}, "Invalid card table ID"},
		{"todos create bad todoset id", NewTodosCmd,
