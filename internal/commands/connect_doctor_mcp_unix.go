...
func mcpHandshakeCheck(ctx context.Context, profile string) setup.Check {
	...
	cmd := exec.Command(exe, "mcp", "--profile", profile) //nolint:gosec // this binary, with a validated profile name
	...
	c.Status, c.Message = setup.StatusPass, fmt.Sprintf("basecamp mcp -P %s answered with %d tools", profile, tools)
	return c
}
