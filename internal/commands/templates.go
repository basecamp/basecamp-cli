...
to check its progress. Without --name the template takes the source's title.`, kind.noun, kind.group),
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// Unset stays off the wire so the server default applies, which also
			// makes --copy-comments=false mean false rather than nothing.
			req := &basecamp.CreateTemplatificationRequest{TemplateName: name}
			if cmd.Flags().Changed("copy-comments") {
				req.CopyComments = &copyComments
			}
			if cmd.Flags().Changed("copy-assignments") {
				req.CopyAssignments = &copyAssignments
			}
			if cmd.Flags().Changed("move-cards-to-triage") {
				req.MoveCardsToTriage = &moveCardsToTriage
			}

			save, err := app.Account().Templates().CreateTemplatification(cmd.Context(), bucketID, recordingID, req)
			if err != nil {
				return convertSDKError(err)
			}
