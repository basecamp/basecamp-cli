...
const (
	...
	upgradeStagePrefix = ".basecamp-upgrade-"
...
func acquireUpgradeLock(target string) (*flock.Flock, error) {
	...
	if !locked {
		return nil, errors.New("another basecamp upgrade is already running")
	}
	return lock, nil
}
...
func releaseWorkflowIdentity(ver string) string {
	return "https://github.com/basecamp/basecamp-cli/.github/workflows/release.yml@refs/tags/v" + ver
}

func releaseTagURL(ver string) string {
	return "https://github.com/basecamp/basecamp-cli/releases/tag/v" + ver
}

const installerHint = "curl -fsSL https://basecamp.com/install-cli | bash"
...
func selfUpdateIneligibility(target string) (reason, hint string) {
	...
	if err != nil || home == "" || !pathWithin(home, target) {
		return "system_path",
			"This binary is outside your home directory. Upgrade with the package manager that installed it, or reinstall under your home: " + installerHint
	}
...
// Returns app.OK on confirmed success; every other outcome is a *output.Error.
func runNativeSelfUpdate(ctx context.Context, ok func(any, ...output.ResponseOption) error, w io.Writer, target, current string, release releaseInfo) error {
	latest := release.Version

	ext, binName := "tar.gz", "basecamp"
	if runtime.GOOS == "windows" {
		ext, binName = "zip", "basecamp.exe"
	}
	archiveName := fmt.Sprintf("basecamp_%s_%s_%s.%s", latest, runtime.GOOS, runtime.GOARCH, ext)
	...
	tmpDir, err := os.MkdirTemp("", "basecamp-upgrade-")
	if err != nil {
	...
	if err := bundleVerifier(checksums, bundleBytes, latest); err != nil {
		fmt.Fprintln(w, "failed")
		return errUpgradeFailed(fmt.Sprintf("signature verification failed for checksums.txt: %v", err))
	}
	...
	if reported, probeErr := binaryVersionProber(ctx, target); probeErr != nil || reported != latest {
		fmt.Fprintln(w, "failed")
...
func errUpgradeFailed(msg string) *output.Error {
	return errUpgradeFailedHint(msg, "The existing install was left in place. Re-run basecamp upgrade, or reinstall: "+installerHint)
}
...
func verifySigstoreBundle(checksums, bundleBytes []byte, ver string) error {
	...
	return verifyBundleWithRoot(trusted, checksums, bundleBytes, ver)
}

func verifyBundleWithRoot(trusted root.TrustedMaterial, checksums, bundleBytes []byte, ver string) error {
	var b bundle.Bundle
	...
	certID, err := verify.NewShortCertificateIdentity(sigstoreOIDCIssuer, "", releaseWorkflowIdentity(ver), "")
	if err != nil {
		return err
	}
...
func sigstoreTrustedRoot() (root.TrustedMaterial, error) {
	...
	if cacheDir, err := os.UserCacheDir(); err == nil {
		opts.CachePath = filepath.Join(cacheDir, "basecamp", "sigstore-tuf")
	}
...
func extractTarGzMember(archivePath, member, dest string) error {
	f, err := os.Open(archivePath) //nolint:gosec // G304: path is our own temp download
	if err != nil {
		return err
	}
	defer f.Close()
...
func copyFileSync(src, dst string) error {
	in, err := os.Open(src) //nolint:gosec // G304: src is the running executable
	if err != nil {
		return err
	}
	defer in.Close()
...
// `.basecamp-upgrade-*` namespace) next to the running executable. Runs on
// every ordinary CLI invocation so a crashed upgrade's staging files — and
// backups an upgrade explicitly marked safe to reap — don't linger.
...
func cleanupUpgradeSidecarsFor(exe string) {
	...
	//
	// Only the `.basecamp-upgrade-*` staging namespace is ever reaped — a
	// positive safe-to-reap contract: upgrades move a verified-garbage backup
	// into this namespace themselves (discardBackup). `.old*` and
	// `.recovered-*` names are NEVER reaped here, because after a failed
	// rollback they can be the user's only good binary.
