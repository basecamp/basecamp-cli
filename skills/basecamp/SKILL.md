...
basecamp connect setup -P agent --operator-profile <me> --route <project-id>=<dir>  # Set up a local agent connector: connects the agent if needed, writes connect.json (trust, routes), checks token, identity, ticket mint and project reads
```
