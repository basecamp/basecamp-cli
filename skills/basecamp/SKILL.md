...
**Avoiding interactive prompts.** Only `--agent`/`--json`/`--quiet`/`--ids-only`/`--count` suppress interactive selection prompts. `--md` does **not** — if a required target is ambiguous (e.g. a project with multiple todosets and no `--todoset`), and the CLI is attached to a terminal, it will show a blocking picker. When you need Markdown output *and* no prompts, either pass the disambiguating flag (`--todoset <id>`, `--list <id>`, `--in <project>`) or set `BASECAMP_NONINTERACTIVE=1` in the environment. `BASECAMP_NONINTERACTIVE` disables all prompts (they become actionable errors instead) without changing the output format — an escape hatch for agents running under a PTY.
