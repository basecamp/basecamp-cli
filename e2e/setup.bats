...
# run_guarded runs a shell snippet under a timeout, with pipefail set so a
# timeout inside a pipeline is not masked by the exit status of the last stage.
run_guarded() {
  local to
  to="$(timeout_bin)"
  if [[ -z "$to" ]]; then
  ...
  run bash -c "set -o pipefail; $to 10 $1"
}
